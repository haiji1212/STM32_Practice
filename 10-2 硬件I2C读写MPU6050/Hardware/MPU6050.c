...
void MPU6050_WaitEvent(I2C_TypeDef* I2Cx, uint32_t I2C_EVENT){	//MPU6050等待事件
	uint32_t TimeOut;
	TimeOut = 10000;	//给定超时时间
	while(I2C_CheckEvent(I2Cx, I2C_EVENT) != SUCCESS){	//循环等待事件
		TimeOut --;
...
uint8_t MPU6050_ReadReg(uint8_t RegAddress){	//读寄存器
	...
	MPU6050_WaitEvent(I2C2, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED);	//等待EV6
	I2C_SendData(I2C2, RegAddress);	//硬件I2C发送寄存器地址
	MPU6050_WaitEvent(I2C2, I2C_EVENT_MASTER_BYTE_TRANSMITTING);	//等待EV8_2
	I2C_GenerateSTART(I2C2, ENABLE);	//硬件I2C重复生成起始条件
	MPU6050_WaitEvent(I2C2, I2C_EVENT_MASTER_MODE_SELECT);	//等待EV5
...
void MPU6050_Init(void){
	...
	I2C_InitStructure.I2C_AcknowledgedAddress = I2C_AcknowledgedAddress_7bit;	//应答地址：7位
	I2C_InitStructure.I2C_ClockSpeed = 50000;	//时钟速度：50KHz
	I2C_InitStructure.I2C_DutyCycle = I2C_DutyCycle_2;	//时钟占空比，Tlow/Thigh = 2
	I2C_InitStructure.I2C_Mode = I2C_Mode_I2C;	//模式：I2C模式
	I2C_InitStructure.I2C_OwnAddress1 = 0x00;	//自身模式
