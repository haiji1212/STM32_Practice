...
uint8_t Serial_TxPacket[4];	//全局变量，保存发送数据包
uint8_t Serial_RxPacket[4];	//全局变量，保存接收数据包
...
void Serial_Init(void)
{
	...
	USART_Init(USART1, &USART_InitStructure);	//配置UASRT1
...
void Serial_SendPacket(void)	//封装包头包尾,发送数据包
{
	Serial_SendByte(0xFF);	//发送包头
	Serial_SendArray(Serial_TxPacket,4);	//发送实际数据包
...
void USART1_IRQHandler(void)	//USART1中断函数
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) == SET)	//由USART1触发的中断
	{
		...
		else if (RxState == 1)	//当前状态为1，接收实际的数据包数据
		{
			...
			pRxPacket ++;	//数据包位置自增
			if (pRxPacket >= 4)	//收到4个数据
			{
				RxState = 2;	//状态跳到2
			}
		}
		else if (RxState == 2)	//当前状态为2，接收包尾
		{
			if (RxData == 0xFE)	//判断数据是否为包尾0xFE
			{
